...
int mdss_livedisplay_parse_dt(struct device_node *np, struct mdss_panel_info *pinfo)
{
	...
	of_property_read_string(np, "qcom,mdss-dsi-panel-version", &panel_version);
	if(strcmp(panel_version, "S6E3FA5") == 0) {
		...
		mlc->dci_p3_supported = true;
		printk(KERN_INFO "DCI_P3 commands read\n");
	} else {
		mlc->dci_p3_supported = false;
		printk(KERN_INFO "This panel not supported DCI_P3 with %s\n", panel_version);
	}
