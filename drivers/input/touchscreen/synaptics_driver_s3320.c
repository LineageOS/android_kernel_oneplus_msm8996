...
Left2RightSwip:%d Right2LeftSwip:%d Up2DownSwip:%d Down2UpSwip:%d Mgestrue:%d Wgestrue:%d\n",
		...
		right_swipe_enable,left_swipe_enable,down_swipe_enable,up_swipe_enable,Mgestrue_gesture,letter_w_enable);
	if ((gesture == DouTap && double_tap_enable) || (gesture == RightVee && right_arrow_enable)
		...
		|| (gesture == Down2UpSwip && up_swipe_enable) || (gesture == Mgestrue && Mgestrue_gesture)
...
static void gesture_enable(struct synaptics_ts_data *ts)
{
	ts->gesture_enable = double_tap_enable || letter_o_enable || down_arrow_enable || up_arrow_enable
			...
			|| letter_w_enable || Mgestrue_gesture ? 1 : 0;
}
...
static int init_synaptics_proc(void)
{
	...
	prEntry_tmp = proc_create("letter_m_enable", 0666, prEntry_tp, &letter_m_enable_proc_fops);
	if(prEntry_tmp == NULL){
		ret = -ENOMEM;
		TPD_ERR("Couldn't create letter_m_enable\n");
	}

	prEntry_tmp = proc_create("letter_w_enable", 0666, prEntry_tp, &letter_w_enable_proc_fops);
	if(prEntry_tmp == NULL){
		ret = -ENOMEM;
